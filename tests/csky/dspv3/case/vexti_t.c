...
{
    int i = 0;
    init_testsuite("Testing insn vexti\n");

    for (i = 0; i < sizeof(dst0) / sizeof(struct vdsp_reg); i++) {
        if (imm[i] == 0x0) {
            test_vexti_0x0(src0[0].fixs8, src1[0].fixs8, res.fixs8);
        } else if (imm[i] == 0x16) {
            test_vexti_0x16(src0[0].fixs8, src1[0].fixs8, res.fixs8);
        } else if (imm[i] == 0x21) {
            test_vexti_0x21(src0[0].fixs8, src1[0].fixs8, res.fixs8);
        } else if (imm[i] == 0x3e) {
            test_vexti_0x3e(src0[0].fixs8, src1[0].fixs8, res.fixs8);
        }
