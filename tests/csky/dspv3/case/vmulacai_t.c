...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x000000000c2c341a, 0x000000001352c899, },
        .fixs32 = {0x000033a1, 0x000024a9, 0xffffd10c, 0x00000ff0, },
       .fixu64 = {0x00000000be430dfb, 0x00000000c5464c36, },
       .fixu32 = {0x0000b1ff, 0x0000a148, 0x000118f3, 0x000074dd, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vmulacai\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulacai_s8(src0[i].fixs8, src1[i].fixs8, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulacai_s16(src0[i].fixs16, src1[i].fixs16, res.fixs64);
        result_compare_s64(res.fixs64, dst0[i].fixs64);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulacai_u8(src0[i].fixu8, src1[i].fixu8, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulacai_u16(src0[i].fixu16, src1[i].fixu16, res.fixu64);
