...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0xc6a630b347e7377b, 0x3970b5993ab1f212, },
        .fixs32 = {0xd6b9745a, 0x15fc5221, 0x8262f661, 0xcfe06686, },
        .fixs16 = {0xa164, 0x475d, 0x4d3f, 0xa349, 0x0ea1, 0x10ef, 0x51f2, 0xd6d6, },
        .fixs8 = {0x21, 0x23, 0xab, 0xfd, 0x91, 0x3c, 0x19, 0x60, 0x7a, 0x92, 0x75, 0xcd, 0x74, 0x8c, 0x34, 0xa5, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vrevq.t\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrevq(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrevh(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrevw(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrevd(src0[i].fixs64, src1[i].fixs64, res.fixs64);
