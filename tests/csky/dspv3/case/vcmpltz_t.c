...
{
    int i = 0;
    init_testsuite("Testing insn vcmpltz.t\n");

    for (i = 0; i < sizeof(src) / sizeof(struct vdsp_reg); i++) {
        test_vcmpltz_s8(src[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst[i].fixs8);
    }

    for (i = 0; i < sizeof(src) / sizeof(struct vdsp_reg); i++) {
        test_vcmpltz_s16(src[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst[i].fixs16);
    }

    for (i = 0; i < sizeof(src) / sizeof(struct vdsp_reg); i++) {
        test_vcmpltz_s32(src[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst[i].fixs32);
    }

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src) / sizeof(struct vdsp_reg); i++) {
        test_vcmpltz_f32(src[i].float32, res.float32);
        result_compare_f32(res.float32, dst[i].float32);
    }

    for (i = 0; i < sizeof(src) / sizeof(struct vdsp_reg); i++) {
        test_vcmpltz_f16(src[i].float16, res.float16);
