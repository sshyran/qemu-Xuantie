...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0x20f773dd, 0x80000000, 0xd95b91ed, 0xaa0a13e6, },
        .fixs16 = {0xcc06, 0xbc49, 0xf16d, 0x7cae, 0x7fff, 0xb8a5, 0xafeb, 0x8000, },
        .fixs8 = {0x78, 0xf8, 0xe4, 0xa8, 0x53, 0xb2, 0x8c, 0x80, 0x8b, 0x32, 0x80, 0x7e, 0x1c, 0xbf, 0x1f, 0xad, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vrmulxas.t.rs\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxas_s8_rs(src0[i].fixs8, src1[i].fixs8, src2[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxas_s16_rs(src0[i].fixs16, src1[i].fixs16, src2[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxas_s32_rs(src0[i].fixs32, src1[i].fixs32, src2[i].fixs32, res.fixs32);
