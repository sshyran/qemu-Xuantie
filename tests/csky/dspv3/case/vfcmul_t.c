...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd1205b8a, 0x50db45b5, 0xd0265578, 0xd065aefb, },
        .float16 = {0xdb91, 0x592c, 0xd3d9, 0xd56b, 0xda72, 0x5868, 0xd2b0, 0xd49e, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfcmul\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmul_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmul_f16(src0[i].float16, src1[i].float16, src2[i].float16);
