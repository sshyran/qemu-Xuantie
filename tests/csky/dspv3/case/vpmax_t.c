...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x483471f7, 0x47ad69d9, 0x483c7bbf, 0x481eefee, },
        .float16 = {0x4aee, 0x46a9, 0x4a65, 0x4626, 0x4b3d, 0x4a1a, 0x4aae, 0x49a2, },
        .fixs32 = {0xcfe06686, 0x15fc5221, 0x7794d541, 0x67e3b37f, },
        .fixs16 = {0x51f2, 0x10ef, 0x4d3f, 0x475d, 0x4fb8, 0x6744, 0xce81, 0x301c, },
        .fixs8 = {0x34, 0x74, 0x75, 0x7a, 0x60, 0x3c, 0xfd, 0x23, 0x21, 0x3a, 0x09, 0x15, 0x2c, 0xac, 0x28, 0x4f, },
        .fixu32 = {0xc5cf5eb9, 0xf0bec11d, 0xe391e1de, 0x888fed5e, },
        .fixu16 = {0xfe56, 0xf166, 0x6f16, 0xdeb9, 0x6371, 0x7e26, 0xd612, 0xfc48, },
        .fixu8 = {0xc2, 0x99, 0xe3, 0x89, 0xfb, 0xad, 0xe6, 0xc9, 0x42, 0xd3, 0x94, 0xa3, 0xae, 0x41, 0x59, 0x9b, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vpmax.t\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmax_s8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmax_s16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmax_s32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmax_u8(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmax_u16(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmax_u32(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmax_f32(src0[i].float32, src1[i].float32, res.float32);
        result_compare_f32(res.float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmax_f16(src0[i].float16, src1[i].float16, res.float16);
