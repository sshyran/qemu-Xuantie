...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd0e74e10, 0x511c12fc, 0x508428fa, 0xcfcd7cf6, },
        .float16 = {0xd975, 0x5b5e, 0x563c, 0xd0d8, 0xd8a6, 0x5a46, 0x5550, 0xd022, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfcmulc\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmulc_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmulc_f16(src0[i].float16, src1[i].float16, src2[i].float16);
