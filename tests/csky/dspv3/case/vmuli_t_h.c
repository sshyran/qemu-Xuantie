...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0xe985554d, 0xc552f919, 0x0a45125d, 0xecb835ea, },
        .fixs16 = {0xf32e, 0x1984, 0x0545, 0x048e, 0xe320, 0x180d, 0x1638, 0xe289, },
        .fixs8 = {0xf4, 0x06, 0xf1, 0x0e, 0xf9, 0x0f, 0xf1, 0x0f, 0x0c, 0x03, 0x07, 0xf1, 0xff, 0xf5, 0x04, 0x04, },
        .fixu32 = {0xafd799f9, 0x68a939c1, 0xd60255b0, 0x5caa650d, },
        .fixu16 = {0x62cb, 0x1d48, 0x5dc5, 0x0364, 0x2b26, 0x0561, 0x5683, 0x4444, },
        .fixu8 = {0x15, 0x04, 0x10, 0x06, 0x18, 0x0e, 0x07, 0x0e, 0x16, 0x1b, 0x12, 0x11, 0x19, 0x04, 0x02, 0x15, },
    },
};
struct vdsp_reg res;
...
{
    int i = 0;
    init_testsuite("Testing insn vmuli.t.h\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_s8_h(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_s16_h(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_s32_h(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_u8_h(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_u16_h(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_u32_h(src0[i].fixu32, src1[i].fixu32, res.fixu32);
