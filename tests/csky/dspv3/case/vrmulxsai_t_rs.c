...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0x3cf00fb7, 0x46c1830b, 0xd7de1ed8, 0xf219d96d, },
        .fixs16 = {0x15e6, 0x8000, 0xf767, 0x67c5, 0x66ae, 0xa668, 0xa8f7, 0xc1bf, },
        .fixs8 = { 0x3c, 0xde, 0xd6, 0xf2, 0x17, 0xaf, 0x80, 0xbb, 0x81, 0x23, 0xf3, 0x52, 0x6e, 0xf0, 0xfa, 0x90, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vrmulxsai.t.rs\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxsai_s8_rs(src0[i].fixs8, src1[i].fixs8, src2[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxsai_s16_rs(src0[i].fixs16, src1[i].fixs16, src2[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxsai_s32_rs(src0[i].fixs32, src1[i].fixs32, src2[i].fixs32, res.fixs32);
