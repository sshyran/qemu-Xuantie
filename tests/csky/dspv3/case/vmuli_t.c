...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd10a0149, 0xcfb9c086, 0x504cc0dc, 0xd084a08d, },
        .float16 = {0xda83, 0xd062, 0x54d5, 0xd643, 0xda03, 0xd00c, 0x5476, 0xd5c7, },
        .fixs32 = {0x0cbf8606, 0xf93343a1, 0x577a4f61, 0xeced6cda, },
        .fixs16 = {0x73d0, 0x93f0, 0xecc8, 0x32b8, 0xe378, 0xf648, 0xfdd8, 0xdbe0, },
        .fixs8 = {0x45, 0xb4, 0x0c, 0xf4, 0x6d, 0x15, 0xd2, 0xba, 0x60, 0x39, 0xbc, 0xb1, 0x9d, 0x0b, 0x83, 0x41, },
        .fixu32 = {0x7dddbd6e, 0xa9c9e1a0, 0x8592f426, 0x2cf7b276, },
        .fixu16 = {0x85f6, 0x8fb3, 0x0006, 0x8efc, 0x8ab6, 0xceab, 0xdaa9, 0x0cde, },
        .fixu8 = {0x38, 0x7c, 0xbc, 0xc8, 0xd4, 0x38, 0xc4, 0xfc, 0x6c, 0x74, 0xec, 0x48, 0x28, 0x44, 0x30, 0xfc, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vmuli\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_s8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_s16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_s32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_u8(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_u16(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_u32(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_f32(src0[i].float32, src1[i].float32, res.float32);
        result_compare_f32(res.float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmuli_f16(src0[i].float16, src1[i].float16, res.float16);
