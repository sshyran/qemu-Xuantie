...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd10a0134, 0x5104daff, 0x504cc091, 0xd0451d67, },
        .float16 = {0xda4f, 0x5a77, 0x5479, 0xd557, 0xd9d3, 0x59f7, 0x5421, 0xd4ee, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfmulxaai\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmulxaai_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmulxaai_f16(src0[i].float16, src1[i].float16, src2[i].float16);
