...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x48bc1e0c, 0xc81e1fba, 0xc6a0a786, 0xc7907603, },
        .float16 = {0x4f3a, 0xca12, 0xbe2c, 0xc58b, 0x4eab, 0xc99a, 0xbdb4, 0xc51e, },
        .fixs64 = {0x319ea006b0111a5c, 0xbf8dcf5161ed4f6c, },
        .fixs32 = {0x584b9145, 0x90a34a4c, 0xae189ea2, 0xc3da35de, },
        .fixs16 = {0x871e, 0x183f, 0xa9ab, 0x1409, 0xd4c8, 0xb3c2, 0x1741, 0xd1af, },
        .fixs8 = {0x84, 0x7e, 0xff, 0x3a, 0xeb, 0x6c, 0x02, 0x65, 0x34, 0x06, 0x90, 0x0b, 0xd5, 0xdd, 0xd4, 0x66, },
        .fixu64 = {0xc37de51271b63238, 0x1164e184199f72d3, },
        .fixu32 = {0xe23d7cdb, 0xc2fb9e91, 0x682ed3bf, 0x4c029377, },
        .fixu16 = {0x9ae5, 0x101e, 0xa779, 0x8a96, 0xd075, 0x37c9, 0x8227, 0xb376, },
        .fixu8 = {0xa6, 0xe7, 0xc6, 0xeb, 0x4f, 0x2b, 0xcc, 0xe6, 0x1f, 0x65, 0x6c, 0x76, 0xac, 0xce, 0x79, 0x59, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vsub\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_s8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_s16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_s32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_s64(src0[i].fixs64, src1[i].fixs64, res.fixs64);
        result_compare_s64(res.fixs64, dst0[i].fixs64);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_u8(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_u16(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_u32(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_u64(src0[i].fixu64, src1[i].fixu64, res.fixu64);
        result_compare_u64(res.fixu64, dst0[i].fixu64);
    }

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_f32(src0[i].float32, src1[i].float32, res.float32);
        result_compare_f32(res.float32, dst0[i].float32);
    }
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_f16(src0[i].float16, src1[i].float16, res.float16);
