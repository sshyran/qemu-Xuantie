...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x000000000c2c341a, 0xfffffffffffeb011, },
        .fixs32 = {0x000033a1, 0x00004439, 0x00003391, 0x0000120a, },
        .fixu64 = {0x00000000be430dfb, 0x000000014e25732e, },
        .fixu32 = {0x0000b1ff, 0x000128c2, 0x00016305, 0x0000a5b7, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vmulaca\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulaca_s8(src0[i].fixs8, src1[i].fixs8, res.fixs32 );
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulaca_s16(src0[i].fixs16, src1[i].fixs16, res.fixs64);
        result_compare_s64(res.fixs64, dst0[i].fixs64);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulaca_u8(src0[i].fixu8, src1[i].fixu8, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulaca_u16(src0[i].fixu16, src1[i].fixu16, res.fixu64);
