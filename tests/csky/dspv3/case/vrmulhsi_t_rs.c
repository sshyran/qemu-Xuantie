...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0x5be91730, 0x46c1830b, 0xbebb8481, 0xf219d96d, },
        .fixs16 = {0x0a99, 0x8000, 0xe643, 0x67c5, 0x7d9a, 0xa668, 0xa72b, 0xc1bf, },
        .fixs8 = {0x71, 0xde, 0x37, 0xf2, 0x68, 0xaf, 0xbc, 0xbb, 0x80, 0x23, 0xa4, 0x52, 0x3e, 0xf0, 0x04, 0x90, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vrmulhsi.t.rs\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulhsi_s8_rs(src0[i].fixs8, src1[i].fixs8, src2[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulhsi_s16_rs(src0[i].fixs16, src1[i].fixs16, src2[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulhsi_s32_rs(src0[i].fixs32, src1[i].fixs32, src2[i].fixs32, res.fixs32);
