...
struct vdsp_reg src0[] = {
    {
        ...
        .fixs32 = {0xffff1111, 0xffff1111, 0x22221111, 0x22221111,  },
        .fixs16 = {0xff11, 0x2211, 0x8111, 0x8211, 0x2211, 0x2211, 0x2211, 0x2211, },
        .fixu64 = {0x2222222211111111, 0x2222222211111111, },
        .fixu32 = {0x0fff1111, 0x0fff1811, 0x22221111, 0x22221111,  },
        .fixu16 = {0xff11, 0x2211, 0x0211, 0x0218, 0x2211, 0x2211, 0x2211, 0x2211, },
    },
};
...
struct vdsp_reg dst0[] = {
    {
        ...
        .fixs32 = {0xfff1fff1, 0x7fff7fff, 0x33333333, 0x33333333, },
        .fixs16 = {0x7ff1, 0x8080, 0x7f7f, 0x7f7f, 0x3333, 0x3333, 0x3333, 0x3333, },
        .fixu64 = {0xffffffffffffffff, 0x3333333333333333, },
        .fixu32 = {0xfff1fff1, 0xffffffff, 0x33333333, 0x33333333, },
        .fixu16 = {0xffff, 0x2121, 0xffff, 0xffff, 0x3333, 0x3333, 0x3333, 0x3333, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vshri.t.ls\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshri_s16_ls(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshri_s32_ls(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshri_s64_ls(src0[i].fixs64, src1[i].fixs64, res.fixs64);
        result_compare_s64(res.fixs64, dst0[i].fixs64);
    }


    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshri_u16_ls(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshri_u32_ls(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshri_u64_ls(src0[i].fixu64, src1[i].fixu64, res.fixu64);
