...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0x23ba9de4, 0xba11513b, 0x3ef002d0, 0xf4cc596b, },
        .fixs16 = {0x1347, 0x45d3, 0x3c1a, 0x049d, 0xb8e5, 0xf35e, 0x3bbd, 0xb88d, },
        .fixs8 = {0xe3, 0xf5, 0x8d, 0x57, 0xd8, 0x3f, 0x91, 0x48, 0x46, 0x16, 0xf4, 0x8c, 0x13, 0xbd, 0x39, 0xee, },
        .fixu32 = {0xd4b0a04c, 0x943eace8, 0xbca7573e, 0x423cbaba, },
        .fixu16 = {0xb0e4, 0x4354, 0x9daa, 0x4371, 0x86dc, 0x71f7, 0x9da6, 0xd603, },
        .fixu8 = {0x6f, 0x36, 0xb6, 0x49, 0xbc, 0x6d, 0x61, 0x96, 0xbe, 0xc9, 0x77, 0x63, 0x90, 0x40, 0x58, 0x9d, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vaddh.t.r\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vaddh_s8_r(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vaddh_s16_r(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vaddh_s32_r(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vaddh_u8_r(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vaddh_u16_r(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vaddh_u32_r(src0[i].fixu32, src1[i].fixu32, res.fixu32);
