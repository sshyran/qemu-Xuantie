...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x50e74e10, 0xd11c12fc, 0xd08428fa, 0x4fcd7cf6, },
        .float16 = {0x5975, 0xdb5e, 0xd63c, 0x50d8, 0x58a6, 0xda46, 0xd550, 0x5022, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfcmulcn\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmulcn_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmulcn_f16(src0[i].float16, src1[i].float16, src2[i].float16);
