...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd10a015e, 0x4fb2d165, 0x4f43fa23, 0x50575446, },
        .float16 = {0xdab8, 0x4ee4, 0x4e0f, 0x55c5, 0xd9bd, 0x4dc2, 0x4d44, 0x54f7, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfnmuls\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfnmuls_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfnmuls_f16(src0[i].float16, src1[i].float16, src2[i].float16);
