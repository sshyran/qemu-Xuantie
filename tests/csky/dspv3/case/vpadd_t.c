...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x4852cdfc, 0x469e3690, 0xc5e9cc60, 0x47e02d88, },
        .float16 = {0x4c0c, 0x3e14, 0x4b78, 0x3d9c, 0xb880, 0x484e, 0xb830, 0x47f2, },
        .fixs32 = {0x52435ce7, 0xecb5c67b, 0x69548156, 0x7ac2f1fb, },
        .fixs16 = {0x28c8, 0x1f90, 0xf088, 0xe8c1, 0x896b, 0x61dc, 0x67fe, 0xffd1, },
        .fixs8 = {0xd9, 0x00, 0x42, 0x0c, 0x79, 0xcd, 0xa8, 0x44, 0xd7, 0xc7, 0xeb, 0xa5, 0x3f, 0x32, 0xf6, 0x0a, },
        .fixu32 = {0x3b8bdae9, 0x58fcc592, 0x9652bf7d, 0xa4cb5e5c, },
        .fixu16 = {0x49b9, 0xfa22, 0x7cf1, 0x8e77, 0x9eb6, 0xc813, 0x74b3, 0x58da, },
        .fixu8 = {0xeb, 0xd7, 0x65, 0xd0, 0xc8, 0x4b, 0x0d, 0xdd, 0x5e, 0x26, 0xeb, 0x1e, 0x44, 0x69, 0x93, 0x0b, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vpadd\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_s8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_s16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_s32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_u8(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_u16(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_u32(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }
#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_f32(src0[i].float32, src1[i].float32, res.float32);
        result_compare_f32(res.float32, dst0[i].float32);
    }
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_f16(src0[i].float16, src1[i].float16, res.float16);
