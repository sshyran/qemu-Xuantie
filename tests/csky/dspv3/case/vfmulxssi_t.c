...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x510a015e, 0xd104dad7, 0xd04cc127, 0x50451c48, },
        .float16 = {0x5ab8, 0xda13, 0xd531, 0x53ec, 0x5a33, 0xd99b, 0xd4cb, 0x5351, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfmulxssi\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmulxssi_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmulxssi_f16(src0[i].float16, src1[i].float16, src2[i].float16);
