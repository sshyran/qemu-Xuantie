...
struct vdsp_reg src1[] = {
    {
        .fixs16 = {0x4fb8, 0x39b3, 0x6744, 0xfa98, 0x0001, 0xffff, 0xffff, 0x0001, },
...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x000000000c2c341b, 0xfffffffffffeb010, },
        .fixs32 = {0x000033a2, 0x80004439, 0x80003390, 0x00001209, },
        .fixu64 = {0x00000000be430dfb, 0x000000014e25732c, },
        .fixu32 = {0x0000b20e, 0x800128c4, 0x80016304, 0x0000a5b6, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vmulacaa\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulacaa_s8(src0[i].fixs8, src1[i].fixs8, src2[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulacaa_s16(src0[i].fixs16, src1[i].fixs16, src2[i].fixs64, res.fixs64);
        result_compare_s64(res.fixs64, dst0[i].fixs64);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulacaa_u8(src0[i].fixu8, src1[i].fixu8, src2[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulacaa_u16(src0[i].fixu16, src1[i].fixu16, src2[i].fixu64, res.fixu64);
