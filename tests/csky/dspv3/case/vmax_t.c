...
struct vdsp_reg src1[] = {
    {
        ...
        .float16 = {0x4aee, 0x4b3d, 0xc332, 0x4a1a, 0x4a65, 0x4aae, 0xc2a4, 0x49a2, },
...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x483471f7, 0x483c7bbf, 0xc73b64a7, 0x481eefee, },
        .float16 = {0x4aee, 0x4b3d, 0xc332, 0x4a1a, 0x4a65, 0x4aae, 0xc2a4, 0x49a2, },
        .fixs32 = {0x7794d541, 0xf1bfac15, 0x67e3b37f, 0x12df3e7c, },
        .fixs16 = {0x4fb8, 0x51f2, 0x6744, 0x0ea1, 0xce81, 0x4d3f, 0x475d, 0xcfb5, },
        .fixs8 = {0x21, 0x34, 0x8d, 0x74, 0xe2, 0x75, 0x92, 0x7a, 0x60, 0x19, 0x3c, 0x91, 0x28, 0xce, 0x4f, 0x21, },
        .fixu32 = {0xe391e1de, 0xb2c0dd9f, 0xf0bec11d, 0x683e0475, },
        .fixu16 = {0xfe56, 0x4b63, 0xf166, 0x7e26, 0x9ea1, 0xd612, 0xdeb9, 0xfc48, },
        .fixu8 = {0xc2, 0x42, 0xd3, 0x53, 0xe3, 0x82, 0x7b, 0xa3, 0xcd, 0xfb, 0xad, 0x9e, 0xe6, 0x59, 0x9b, 0xc9, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vmax.t\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmax_s8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmax_s16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmax_s32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmax_u8(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmax_u16(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmax_u32(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmax_f32(src0[i].float32, src1[i].float32, res.float32);
        result_compare_f32(res.float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmax_f16(src0[i].float16, src1[i].float16, res.float16);
