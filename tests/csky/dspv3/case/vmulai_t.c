...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd10a0134, 0xcfb9bfe5, 0x504cc091, 0xd084a0d5, },
        .float16 = {0xda4f, 0xcf37, 0x5479, 0xd6f4, 0xd9d3, 0xceaa, 0x5421, 0xd66a, },
        .fixs32 = {0x3bb347d0, 0xca9ab8e0, 0x2abff89d, 0xb877b21d, },
        .fixs16 = {0x64c6, 0x1d30, 0xdd97, 0xa399, 0x2754, 0xcccc, 0xd175, 0x62b3, },
        .fixs8 = {0x9f, 0x9f, 0x25, 0x04, 0xc8, 0xe2, 0x72, 0x94, 0xef, 0x62, 0x6f, 0xe6, 0xda, 0xe5, 0x90, 0xda, },
        .fixu32 = {0x5499e15d, 0x7bcc175b, 0x61113f55, 0x85854e35, },
        .fixu16 = {0xe37b, 0x3ac3, 0xc756, 0x3a26, 0x3464, 0xc827, 0x1c07, 0x950b, },
        .fixu8 = {0x0b, 0x55, 0x06, 0x99, 0x60, 0x92, 0xb3, 0x52, 0xd8, 0x36, 0x30, 0x0a, 0xbe, 0xe0, 0x53, 0x63, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vmulai\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulai_s8(src0[i].fixs8, src1[i].fixs8, src2[i].fixs8);
        result_compare_s8(src2[i].fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulai_s16(src0[i].fixs16, src1[i].fixs16, src2[i].fixs16);
        result_compare_s16(src2[i].fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulai_s32(src0[i].fixs32, src1[i].fixs32, src2[i].fixs32);
        result_compare_s32(src2[i].fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulai_u8(src0[i].fixu8, src1[i].fixu8, src2[i].fixu8);
        result_compare_u8(src2[i].fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulai_u16(src0[i].fixu16, src1[i].fixu16, src2[i].fixu16);
        result_compare_u16(src2[i].fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulai_u32(src0[i].fixu32, src1[i].fixu32, src2[i].fixu32);
        result_compare_u32(src2[i].fixu32, dst0[i].fixu32);
    }

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulai_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmulai_f16(src0[i].float16, src1[i].float16, src2[i].float16);
