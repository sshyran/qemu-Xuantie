...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd1205b75, 0x50db45dd, 0xd02655c3, 0xd065af8b, },
        .float16 = {0xdb5d, 0x595e, 0xd448, 0xd61b, 0xda42, 0x5896, 0xd35a, 0xd541, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfcmula\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmula_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmula_f16(src0[i].float16, src1[i].float16, src2[i].float16);
