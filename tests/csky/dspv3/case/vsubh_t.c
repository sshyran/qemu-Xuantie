...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0xac25c8a2, 0xc851a526, 0xd70c4f51, 0xe1ed1aef, },
        .fixs16 = {0xc38f, 0x0c1f, 0xd4d5, 0x0a04, 0xea64, 0x59e1, 0x0ba0, 0xe8d7, },
        .fixs8 = {0xc2, 0x3f, 0xff, 0x1d, 0xf5, 0x36, 0x01, 0x32, 0x1a, 0x03, 0x48, 0x05, 0xea, 0xee, 0xea, 0x33, },
        .fixu32 = {0xf11ebe6d, 0xe17dcf48, 0x341769df, 0x260149bb, },
        .fixu16 = {0x4d72, 0x080f, 0x53bc, 0xc54b, 0xe83a, 0x9be4, 0x4113, 0xd9bb, },
        .fixu8 = {0x53, 0xf3, 0xe3, 0xf5, 0x27, 0x15, 0xe6, 0xf3, 0x0f, 0x32, 0x36, 0x3b, 0x56, 0xe7, 0xbc, 0x2c, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vsubh.t\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsubh_s8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsubh_s16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsubh_s32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsubh_u8(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsubh_u16(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsubh_u32(src0[i].fixu32, src1[i].fixu32, res.fixu32);
