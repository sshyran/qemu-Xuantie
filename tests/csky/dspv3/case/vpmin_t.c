...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x46f2e02b, 0xc785dc35, 0xc843ca22, 0xc73b64a7, },
        .float16 = {0x40aa, 0xc524, 0x404e, 0xc4bf, 0xcb85, 0xc332, 0xcaf1, 0xc2a4, },
        .fixs32 = {0x8262f661, 0xd6b9745a, 0xf1bfac15, 0x12df3e7c, },
        .fixs16 = {0xd6d6, 0x0ea1, 0xa349, 0xa164, 0x39b3, 0xfa98, 0x997d, 0xcfb5, },
        .fixs8 = {0xa5, 0x8c, 0xcd, 0x92, 0x19, 0x91, 0xab, 0x21, 0xb6, 0x8d, 0xe2, 0x90, 0x13, 0x86, 0xce, 0xbb, },
        .fixu32 = {0x75bc7c30, 0x683e0475, 0xb2c0dd9f, 0x1c3b70fe, },
        .fixu16 = {0x4b63, 0x08bc, 0x0ddb, 0xafbe, 0x3b45, 0x49ed, 0x9ea1, 0x5c92, },
        .fixu8 = {0x29, 0x3e, 0x82, 0x47, 0xcd, 0x9e, 0x27, 0x14, 0x1c, 0x53, 0x57, 0x7b, 0x96, 0x28, 0x3a, 0x70, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vpmin.t\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmin_s8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmin_s16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmin_s32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmin_u8(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmin_u16(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmin_u32(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmin_f32(src0[i].float32, src1[i].float32, res.float32);
        result_compare_f32(res.float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpmin_f16(src0[i].float16, src1[i].float16, res.float16);
