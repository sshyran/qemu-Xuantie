...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x51205b8a, 0xd0db45b5, 0x50265578, 0x5065aefb, },
        .float16 = {0x5b91, 0xd92c, 0x53d9, 0x556b, 0x5a72, 0xd868, 0x52b0, 0x549e, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfcmuln\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmuln_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmuln_f16(src0[i].float16, src1[i].float16, src2[i].float16);
