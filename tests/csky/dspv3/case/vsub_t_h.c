...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0xbf8dcf51319ea006, 0x0000000000000000, },
        .fixs32 = {0x90a3584b, 0xc3daae18, 0x00000000, 0x00000000, },
        .fixs16 = {0x1887, 0x14a9, 0xb3d4, 0xd117, 0x0000, 0x0000, 0x0000, 0x0000, },
        .fixu64 = {0x1164e184c37de512, 0x0000000000000000, },
        .fixu32 = {0xc2fbe23d, 0x4c02682e, 0x00000000, 0x00000000, },
        .fixu16 = {0x109a, 0x8aa7, 0x37d0, 0xb382, 0x0000, 0x0000, 0x0000, 0x0000, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vsub.t.h\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_s16_h(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_s32_h(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_s64_h(src0[i].fixs64, src1[i].fixs64, res.fixs64);
        result_compare_s64(res.fixs64, dst0[i].fixs64);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_u16_h(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_u32_h(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsub_u64_h(src0[i].fixu64, src1[i].fixu64, res.fixu64);
