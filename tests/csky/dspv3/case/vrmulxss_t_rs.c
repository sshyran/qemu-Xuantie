...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0x5be91730, 0xcc0bd11c, 0xc16d82d7, 0xc84c7164, },
        .fixs16 = {0x0a99, 0x9bce, 0xe326, 0x7198, 0x2002, 0x8c43, 0xb8cb, 0xa1c0, },
        .fixs8 = {0x71, 0xb6, 0xb1, 0x45, 0x4f, 0xd1, 0x80, 0xec, 0x80, 0x1b, 0xda, 0x6e, 0x3e, 0xd9, 0xf7, 0xac, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vrmulxss.t.rs\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxss_s8_rs(src0[i].fixs8, src1[i].fixs8, src2[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxss_s16_rs(src0[i].fixs16, src1[i].fixs16, src2[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxss_s32_rs(src0[i].fixs32, src1[i].fixs32, src2[i].fixs32, res.fixs32);
