...
struct vdsp_reg src2[] = {
    {
        .fixs64 = {0x336fd91768b65fd2, 0x3126935cdf8b7be6, },
...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x35f09591aab0d236, 0x063e703167f3b36b, },
        .fixs32 = {0x5fe45483, 0xa0fafc21, 0x35e61263, 0xd2dd7e7e, },
        .fixs16 = {0xdfde, 0x31f3, 0x17cf, 0x8ab9, 0x8f49, 0x4d7d, 0x631d, 0xc964, },
        .fixs8 = {0x21, 0x34, 0x8c, 0x3a, 0xe9, 0x45, 0x90, 0x5f, 0x20, 0x1b, 0x3c, 0x93, 0x3d, 0x8e, 0x43, 0x23, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vsel.t\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsel_8(src0[i].fixs8, src1[i].fixs8, src2[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsel_16(src0[i].fixs16, src1[i].fixs16, src2[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsel_32(src0[i].fixs32, src1[i].fixs32, src2[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsel_64(src0[i].fixs64, src1[i].fixs64, src2[i].fixs64, res.fixs64);
