...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0x80000000, 0xecb5c67b, 0x69548156, 0x7ac2f1fb, },
        .fixs16 = {0x28c8, 0x1f90, 0xf088, 0xe8c1, 0x7fff, 0x61dc, 0x8000, 0xffd1, },
        .fixs8 = {0xd9, 0x00, 0x42, 0x0c, 0x79, 0xcd, 0xa8, 0x44, 0xd7, 0xc7, 0xeb, 0xa5, 0x3f, 0x80, 0xf6, 0x0a, },
        .fixu32 = {0xffffffff, 0xffffffff, 0xffffffff, 0xa4cb5e5c, },
        .fixu16 = {0xffff, 0xfa22, 0x7cf1, 0xffff, 0x9eb6, 0xc813, 0xffff, 0xffff, },
        .fixu8 = {0xeb, 0xd7, 0xff, 0xd0, 0xff, 0xff, 0xff, 0xdd, 0x5e, 0xff, 0xeb, 0xff, 0xff, 0x69, 0x93, 0xff, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vpadd_s\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_s8_s(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_s16_s(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_s32_s(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_u8_s(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_u16_s(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vpadd_u32_s(src0[i].fixu32, src1[i].fixu32, res.fixu32);
