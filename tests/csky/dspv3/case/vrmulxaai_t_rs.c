...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0x1fe6c64, 0xd6c31962, 0xe7cfcdf7, 0xc9179fb4, },
        .fixs16 = {0xd753, 0x8000, 0xfb5b, 0x7883, 0x0a1e, 0xacb9, 0x000f, 0xa6fe, },
        .fixs8 = {0x43, 0x20, 0xfb, 0x53, 0x4e, 0xea, 0x84, 0x1a, 0xa8, 0xf2, 0xc2, 0x12, 0x3c, 0xdc, 0x16, 0x85, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vrmulxaai.t.rs\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxaai_s8_rs(src0[i].fixs8, src1[i].fixs8, src2[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxaai_s16_rs(src0[i].fixs16, src1[i].fixs16, src2[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxaai_s32_rs(src0[i].fixs32, src1[i].fixs32, src2[i].fixs32, res.fixs32);
