...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x3ea2a00bb01125a4, 0xc1be51d2a21edf74, },
        .fixs32 = {0xb874b3c7, 0x73dd5a74, 0x721fe15e, 0xc4664a26, },
        .fixs16 = {0x996e, 0x6841, 0x77ab, 0xf439, 0x6dc8, 0xd442, 0x7741, 0x6ed1, },
        .fixs8 = {0x84, 0x82, 0x01, 0x4e, 0x2f, 0x7c, 0x02, 0x6f, 0x4c, 0x0a, 0x90, 0x17, 0xd5, 0x65, 0x6c, 0x9a, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vxor\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vxor_8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vxor_16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vxor_32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vxor_64(src0[i].fixs64, src1[i].fixs64, res.fixs64);
