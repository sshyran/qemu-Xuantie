...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd0e74de6, 0x511c1310, 0x508428d5, 0xcfcd7e16, },
        .float16 = {0xd941, 0x5b90, 0x55e1, 0xd239, 0xd876, 0x5a74, 0x54fc, 0xd168, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfcmulca\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmulca_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmulca_f16(src0[i].float16, src1[i].float16, src2[i].float16);
