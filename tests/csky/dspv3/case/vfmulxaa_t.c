...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd10a0134, 0x5104daff, 0x4f43f7cd, 0xd026374b, },
        .float16 = {0xda4f, 0x5a77, 0x4a61, 0xd49c, 0xd95c, 0x5985, 0x493b, 0xd3f5, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfmulxaa\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmulxaa_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmulxaa_f16(src0[i].float16, src1[i].float16, src2[i].float16);
