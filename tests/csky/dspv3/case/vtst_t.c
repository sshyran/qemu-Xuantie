...
struct vdsp_reg src0[] = {
    {
        .fixs64 = {0xa0a0a0a0a0a0a0a0, 0xa0a0a0a0a0a0a0a0, },
...
struct vdsp_reg src1[] = {
    {
        .fixs64 = {0x0505050505050505, 0xa0a0a0a0a0a0a0a0, },
...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x0000000000000000, 0xffffffffffffffff, },
        .fixs32 = {0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, },
        .fixs16 = {0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, },
...
{
    int i = 0;
    init_testsuite("Testing insn vtst.t\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vtst_8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vtst_16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vtst_32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vtst_64(src0[i].fixs64, src1[i].fixs64, res.fixs64);
