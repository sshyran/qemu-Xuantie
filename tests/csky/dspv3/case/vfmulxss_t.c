...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x510a015e, 0xd104dad7, 0xcf43fa23, 0x5026362c, },
        .float16 = {0x5ab8, 0xda13, 0xce0f, 0x5276, 0x59bd, 0xd929, 0xcd44, 0x5169, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfmulxss\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmulxss_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmulxss_f16(src0[i].float16, src1[i].float16, src2[i].float16);
