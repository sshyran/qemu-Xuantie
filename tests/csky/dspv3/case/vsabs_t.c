...
struct vdsp_reg dst0[] = {
    {
        ...
        .float16 = {0x4f3a, 0x4a12, 0x3e2c, 0x458b, 0x4eab, 0x499a, 0x3db4, 0x451e, },
    },
};
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsabs_f32(src0[i].float32, src1[i].float32, res.float32);
        result_compare_f32(res.float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsabs_f16(src0[i].float16, src1[i].float16, res.float16);
