...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0x1fe6c64, 0xdf63c32c, 0xe51dcfa1, 0xc5745c92, },
        .fixs16 = {0xd753, 0xae30, 0xfe78, 0x7043, 0x67b6, 0x98a7, 0xee6f, 0xaa85, },
        .fixs8 = {0x43, 0xcd, 0x7f, 0x45, 0x67, 0xd5, 0x00, 0xee, 0xb0, 0x2d, 0x8c, 0x7f, 0x3c, 0xfb, 0x23, 0x87, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vrmulha.t.rs\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulha_s8_rs(src0[i].fixs8, src1[i].fixs8, src2[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulha_s16_rs(src0[i].fixs16, src1[i].fixs16, src2[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulha_s32_rs(src0[i].fixs32, src1[i].fixs32, src2[i].fixs32, res.fixs32);
