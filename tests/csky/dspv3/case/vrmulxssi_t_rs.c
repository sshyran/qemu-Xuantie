...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0x5be91730, 0xcc0bd11c, 0xbebb8481, 0xcdfcead2, },
        .fixs16 = {0x0a99, 0x9bce, 0xe643, 0x693f, 0x7d9a, 0x004f, 0xa72b, 0x8000, },
        .fixs8 = {0x71, 0xb6, 0x37, 0xcd, 0x68, 0xb0, 0xbc, 0x9a, 0x80, 0x61, 0xa4, 0x58, 0x3e, 0xd8, 0x04, 0xad,},
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vrmulxssi.t.rs\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxssi_s8_rs(src0[i].fixs8, src1[i].fixs8, src2[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxssi_s16_rs(src0[i].fixs16, src1[i].fixs16, src2[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vrmulxssi_s32_rs(src0[i].fixs32, src1[i].fixs32, src2[i].fixs32, res.fixs32);
