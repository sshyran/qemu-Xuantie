...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xcfb2d15d, 0xcfb9bfe4, 0xd07f5b68, 0xd084a0d5, },
        .float16 = {0xced0, 0xcf38, 0xd662, 0xd6f3, 0xce49, 0xcea9, 0xd5e5, 0xd66a, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfmulxasi\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmulxasi_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmulxasi_f16(src0[i].float16, src1[i].float16, src2[i].float16);
