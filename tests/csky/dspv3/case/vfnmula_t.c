...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x510a0134, 0xcfb2d2a8, 0xcf43f7cd, 0xd0575326, },
        .float16 = {0x5a4f, 0xd0ff, 0xca61, 0xd464, 0x595c, 0xd04f, 0xc93b, 0xd363, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfnmula\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfnmula_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfnmula_f16(src0[i].float16, src1[i].float16, src2[i].float16);
