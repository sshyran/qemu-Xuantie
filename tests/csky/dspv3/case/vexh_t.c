...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x11110888b3333333, 0x11110888b3333333, },
        .fixs32 = {0x22113333, 0x22113333, 0x22113333, 0x22113333, },
        .fixs16 = {0x2133, 0x2133, 0x2133, 0x2133, 0x2133, 0x2133, 0x2133, 0x2133, },
        .fixs8 = {0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vexh.t\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vexh_8(src0[i].fixs8, src1[3], src2[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vexh_16(src0[i].fixs16, src1[2], src2[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vexh_32(src0[i].fixs32, src1[1], src2[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vexh_64(src0[i].fixs64, src1[0], src2[i].fixs64, res.fixs64);
