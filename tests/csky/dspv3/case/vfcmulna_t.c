...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x51205b9f, 0xd0db458d, 0x5026552d, 0x5065ae6c, },
        .float16 = {0x5bc6, 0xd8fa, 0x5321, 0x54ba, 0x5aa3, 0xd83a, 0x5207, 0x53f6, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfcmulna\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmulna_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmulna_f16(src0[i].float16, src1[i].float16, src2[i].float16);
