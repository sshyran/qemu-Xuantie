...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0x50e74e3a, 0xd11c12e8, 0xd084291f, 0x4fcd7bd8, },
        .float16 = {0x59aa, 0xdb2c, 0xd698, 0x4eee, 0x58d7, 0xda18, 0xd5a5, 0x4db7, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfcmulcna\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmulcna_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfcmulcna_f16(src0[i].float16, src1[i].float16, src2[i].float16);
