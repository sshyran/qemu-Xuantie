...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x72E16B327563E424, 0xffc6a630b347e737, },
        .fixs32 = {0x9FC0CD0C, 0x80000000, 0x0AFE2910, 0xffffffff, },
        .fixs16 = {0xadac, 0x7fff, 0x10ef, 0x000e, 0xffff, 0x0000, 0x7fff, 0xffff, },
        .fixs8 = {0x80, 0x68, 0x80, 0x3a, 0xff, 0x7f, 0xff, 0x7f, 0x7f, 0x7f, 0x00, 0xff, 0x80, 0xff, 0x7f, 0x00, },
        .fixu64 = {0xffffffffffffffff, 0x1c626bcd, },
        .fixu32 = {0xffffffff, 0xEB78F860, 0x00f0bec1, 0x683e0475, },
        .fixu16 = {0xffff, 0x96C6, 0x00f1, 0x08bc, 0x0000, 0x0000, 0xffff, 0x0000, },
        .fixu8 = {0xff, 0xa4, 0x99, 0x03, 0x00, 0xff, 0xff, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0xff, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vsht.t.s\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsht_s8_s(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsht_s16_s(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsht_s32_s(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsht_s64_s(src0[i].fixs64, src1[i].fixs64, res.fixs64);
        result_compare_s64(res.fixs64, dst0[i].fixs64);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsht_u8_s(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsht_u16_s(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsht_u32_s(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vsht_u64_s(src0[i].fixu64, src1[i].fixu64, res.fixu64);
