...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd10a0134, 0x4fb2d2a8, 0x4f43f7cd, 0x50575326, },
        .float16 = {0xda4f, 0x50ff, 0x4a61, 0x5464, 0xd95c, 0x504f, 0x493b, 0x5363, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfmula\n");

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmula_f32(src0[i].float32, src1[i].float32, src2[i].float32);
        result_compare_f32(src2[i].float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vfmula_f16(src0[i].float16, src1[i].float16, src2[i].float16);
