...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x03970b5993ab1f21, 0x0000000000000000, },
        .fixs32 = {0xfcfe0668, 0x00000000, 0x00000000, 0xd6b9745a, },
        .fixs16 = {0xd6d6, 0x0000, 0x010f,0x0000, 0x0000, 0x0000, 0x0000, 0x0000, },
        .fixs8 = {0xa5, 0x00, 0xf9, 0x00, 0xcd, 0x00, 0xf9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
        .fixu64 = {0x07ff0e92e56fe197, 0x0000000000000000, },
        .fixu32 = {0x0c5cf5ec, 0x00000000, 0x00000000, 0x683e0475,  },
        .fixu16 = {0xfe56, 0x0000, 0x0f16, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,},
        .fixu8 = {0xc2, 0x00, 0x0a, 0x00, 0xe3, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00,  0x00, 0x00, 0x00, 0x00, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vshr_t_r\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshr_s8_r(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshr_s16_r(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshr_s32_r(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshr_s64_r(src0[i].fixs64, src1[i].fixs64, res.fixs64);
        result_compare_s64(res.fixs64, dst0[i].fixs64);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshr_u8_r(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshr_u16_r(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshr_u32_r(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshr_u64_r(src0[i].fixu64, src1[i].fixu64, res.fixu64);
