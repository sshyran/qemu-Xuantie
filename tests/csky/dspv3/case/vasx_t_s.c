...
struct vdsp_reg dst0[] = {
    {
        .fixs32 = {0x11111111, 0x33333333, 0x80000000, 0x7fffffff, },
        .fixs16 = {0x1111, 0x3333, 0x1111, 0x3333, 0x1111, 0x3333, 0x7fff, 0x8000, },
        .fixs8 = {0x11, 0x33, 0x11, 0x33, 0x11, 0x33, 0x11, 0x33, 0x11, 0x33, 0x11, 0x33, 0x11, 0x33, 0x80, 0x7f, },
        .fixu32 = {0x11111111, 0x33333333, 0x00000000, 0xffffffff, },
        .fixu16 = {0x1111, 0x3333, 0x1111, 0x3333, 0x1111, 0x3333, 0xe111, 0xffff, },
        .fixu8 = {0x11, 0x33, 0x11, 0x33, 0x11, 0x33, 0x11, 0x33, 0x11, 0x33, 0x11, 0x33, 0x11, 0x33, 0x00, 0xff, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vasx.t.s\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vasx_s8_s(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vasx_s16_s(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vasx_s32_s(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vasx_u8_s(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vasx_u16_s(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vasx_u32_s(src0[i].fixu32, src1[i].fixu32, res.fixu32);
