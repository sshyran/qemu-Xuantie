...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xc843ca22, 0x46f2e02b, 0xc785dc35, 0x47ad69d9, },
        .float16 = {0xcb85, 0x40aa, 0xc524, 0x46a9, 0xcaf1, 0x404e, 0xc4bf, 0x4626, },
        .fixs32 = {0xcfe06686, 0x8262f661, 0x15fc5221, 0xd6b9745a, },
        .fixs16 = {0xd6d6, 0x39b3, 0x10ef, 0xfa98, 0xa349, 0x997d, 0x301c, 0xa164, },
        .fixs8 = {0xa5, 0xb6, 0x8c, 0x3a, 0xcd, 0x09, 0x90, 0x15, 0x2c, 0x13, 0xac, 0x86, 0xfd, 0xab, 0x23, 0xbb, },
        .fixu32 = {0xc5cf5eb9, 0x75bc7c30, 0x888fed5e, 0x1c3b70fe, },
        .fixu16 = {0x6371, 0x3b45, 0x49ed, 0x08bc, 0x6f16, 0x0ddb, 0x5c92, 0xafbe, },
        .fixu8 = {0x1c, 0x29, 0x99, 0x3e, 0x94, 0x57, 0x47, 0x89, 0xae, 0x96, 0x41, 0x28, 0x3a, 0x27, 0x14, 0x70, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vmin.t\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmin_s8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmin_s16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmin_s32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmin_u8(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmin_u16(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmin_u32(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmin_f32(src0[i].float32, src1[i].float32, res.float32);
        result_compare_f32(res.float32, dst0[i].float32);
    }
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmin_f16(src0[i].float16, src1[i].float16, res.float16);
