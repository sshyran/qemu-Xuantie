...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x3a6af84b8c060d08, 0xc7893cdf84a6747c, },
        .fixs32 = {0xded3012e, 0x809aebe3, 0x22f8c890, 0xd9155c29, },
        .fixs16 = {0xe0cd, 0x5928, 0x1dd7, 0x0df4, 0x9d19, 0x406e, 0x4d60, 0x9b5a, },
        .fixs8 = {0xa9, 0x2a, 0x7d, 0x7b, 0xc9, 0x76, 0x84, 0x7c, 0x65, 0x1b, 0x31, 0x81, 0x02, 0xa4, 0x2c, 0x18, },
        .fixu64 = {0x977f49b1d3a71650, 0x1dc21d1648c465dc, },
        .fixu32 = {0xe2419af4, 0x8c1497e3, 0x01d0bec8, 0x6bc57294, },
        .fixu16 = {0x0ac4, 0x52cb, 0xfaa3, 0x1880, 0x82ea, 0x289d, 0xea4b, 0xcf47, },
        .fixu8 = {0xc5, 0x31, 0xb3, 0x48, 0xf5, 0x8c, 0x56, 0x9d, 0xe2, 0x0d, 0xb5, 0xa3, 0xed, 0x32, 0x27, 0xd7, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vshria\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshria_s8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshria_s16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshria_s32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshria_s64(src0[i].fixs64, src1[i].fixs64, res.fixs64);
        result_compare_s64(res.fixs64, dst0[i].fixs64);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshria_u8(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshria_u16(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshria_u32(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vshria_u64(src0[i].fixu64, src1[i].fixu64, res.fixu64);
