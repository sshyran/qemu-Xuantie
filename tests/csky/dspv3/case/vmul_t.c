...
struct vdsp_reg dst0[] = {
    {
        .float32 = {0xd10a0149, 0x4fb2d206, 0x4f43f8f8, 0x505753b6, },
        .float16 = {0xda83, 0x5038, 0x4ca0, 0x5514, 0xd98c, 0x4f30, 0x4be1, 0x5454, },
        .fixs32 = {0x0cbf8606, 0x530661f5, 0x6fdcd15f, 0x9a682798, },
        .fixs16 = {0x73d0, 0x2e36, 0xa87c, 0xe998, 0x05c9, 0x5ec3, 0x3e2c, 0xf7b4, },
        .fixs8 = {0x45, 0xf8, 0x1c, 0x48, 0xfa, 0x1d, 0x20, 0x02, 0x80, 0xdb, 0x50, 0xe6, 0x88, 0x9a, 0xcd, 0x1b, },
        .fixu32 = {0x7dddbd6e, 0x5b4691d0, 0x4605c1a6, 0x62729c16, },
        .fixu16 = {0x85f6, 0x22af, 0x916e, 0xd3e8, 0x70d6, 0x0b66, 0x8182, 0x7570, },
        .fixu8 = {0x38, 0x92, 0x1b, 0x1a, 0x3c, 0x2e, 0x1d, 0x3b, 0x56, 0x12, 0xed, 0xb0, 0x1c, 0x8f, 0x1c, 0xf0, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vmul\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmul_s8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmul_s16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmul_s32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmul_u8(src0[i].fixu8, src1[i].fixu8, res.fixu8);
        result_compare_u8(res.fixu8, dst0[i].fixu8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmul_u16(src0[i].fixu16, src1[i].fixu16, res.fixu16);
        result_compare_u16(res.fixu16, dst0[i].fixu16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmul_u32(src0[i].fixu32, src1[i].fixu32, res.fixu32);
        result_compare_u32(res.fixu32, dst0[i].fixu32);
    }

#ifdef VDSP_FLOAT
    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmul_f32(src0[i].float32, src1[i].float32, res.float32);
        result_compare_f32(res.float32, dst0[i].float32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vmul_f16(src0[i].float16, src1[i].float16, res.float16);
