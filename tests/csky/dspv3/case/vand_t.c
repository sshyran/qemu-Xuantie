...
struct vdsp_reg dst0[] = {
    {
        .fixs64 = {0x015015900aa0d212, 0x0600202145e1200b, },
        .fixs32 = {0x47804400, 0x8022a401, 0x05e01221, 0x12993458, },
        .fixs16 = {0x4690, 0x11b2, 0x0044, 0x0a80, 0x8201, 0x093d, 0x001c, 0x8124, },
        .fixs8 = {0x21, 0x34, 0x8c, 0x30, 0xc0, 0x01, 0x90, 0x10, 0x20, 0x11, 0x2c, 0x80, 0x28, 0x8a, 0x03, 0x21, },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vand\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vand_8(src0[i].fixs8, src1[i].fixs8, res.fixs8);
        result_compare_s8(res.fixs8, dst0[i].fixs8);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vand_16(src0[i].fixs16, src1[i].fixs16, res.fixs16);
        result_compare_s16(res.fixs16, dst0[i].fixs16);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vand_32(src0[i].fixs32, src1[i].fixs32, res.fixs32);
        result_compare_s32(res.fixs32, dst0[i].fixs32);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct vdsp_reg); i++) {
        test_vand_64(src0[i].fixs64, src1[i].fixs64, res.fixs64);
