...
binary_calculation_64_t samples_fcmphsd[] = {
...
void test_fcmphsd()
{
    ...
    int i = 0;
    for (i = 0;
        i < sizeof(samples_fcmphsd) / sizeof(samples_fcmphsd[0]);
        i++)
    {
        asm ("ldw r2, (%1, 0);\n\t"\
            "ldw r3, (%1, 4);\n\t"\
            ...
            "done:\n\t"
            : "=r"(fcmphsd_psr)
            : "r"(&samples_fcmphsd[i].op1.a.i[0]),
            "r"(&samples_fcmphsd[i].op2.a.i[0])
            : "fr0", "fr1", "r2"
        );
        TEST((fcmphsd_psr & 0x1)
            == samples_fcmphsd[i].result.a.ull);
    }
}
