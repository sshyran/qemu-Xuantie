...
void test_ffnmula_64(float64 a, float64 b, float64 c, float64 expt)
{
    float64 res;
    asm ("fmov.64 %0, %3;\
         ffnmula.64 %0, %1, %2;"
...
{
    int i = 0;
    init_testsuite("Testing fpu ffnmula.64 instructions.\n");

    for (i = 0; i < sizeof(data) / sizeof(struct op3_res1_f64); i++) {
        test_ffnmula_64(data[i].op1, data[i].op2, data[i].op3, data[i].res);
    }
