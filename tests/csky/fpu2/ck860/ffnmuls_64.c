...
struct op3_res1_f64 data[] = {
    ...
    {.op1.f = 0.5,    .op2.d = DpInf,   .op3.f = 0.5,     .res.d = DpInf},
    {.op1.f = -2.5,   .op2.d = DpsNaN,  .op3.f = 0.12,    .res.d = 0x7ff8000000000001},
};

void test_ffnmuls_64(float64 a, float64 b, float64 c, float64 expt)
{
    float64 res;
    asm ("fmov.64 %0, %3;\
         ffnmuls.64 %0, %1, %2;"
...
{
    int i = 0;
    init_testsuite("Testing fpu ffnmuls.64 instructions.\n");

    for (i = 0; i < sizeof(data) / sizeof(struct op3_res1_f64); i++) {
        test_ffnmuls_64(data[i].op1, data[i].op2, data[i].op3, data[i].res);
    }
