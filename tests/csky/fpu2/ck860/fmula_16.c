...
void test_fmula_16(float16 a, float16 b, float16 c, float16 expt)
{
    float16 res;
    asm ("fmov.16 %0, %3;\
         fmula.16 %0, %1, %2;"
...
{
    int i = 0;
    init_testsuite("Testing fpu fmula.16 instructions.\n");

    for (i = 0; i < sizeof(data) / sizeof(struct op3_res1_f16); i++) {
        test_fmula_16(data[i].op1, data[i].op2, data[i].op3, data[i].res);
    }
