...
void test_ffmula_32(float32 a, float32 b, float32 c, float32 expt)
{
    float32 res;
    asm ("fmov.32 %0, %3;\
         ffmula.32 %0, %1, %2;"
...
{
    int i = 0;
    init_testsuite("Testing fpu ffmula.32 instructions.\n");

    for (i = 0; i < sizeof(data) / sizeof(struct op3_res1_f32); i++) {
        test_ffmula_32(data[i].op1, data[i].op2, data[i].op3, data[i].res);
    }
