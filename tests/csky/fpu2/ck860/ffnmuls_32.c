...
struct op3_res1_f32 data[] = {
    ...
    {.op1.f = 0.5,    .op2.d = SpInf,   .op3.f = -0.5,    .res.d = SpInf},
    {.op1.f = -2.5,   .op2.d = SpsNaN,  .op3.f = -0.12,   .res.d = 0x7fc00001},
};

void test_ffnmuls_32(float32 a, float32 b, float32 c, float32 expt)
{
    float32 res;
    asm ("fmov.32 %0, %3;\
         ffnmuls.32 %0, %1, %2;"
...
{
    int i = 0;
    init_testsuite("Testing fpu ffnmuls.32 instructions.\n");

    for (i = 0; i < sizeof(data) / sizeof(struct op3_res1_f32); i++) {
        test_ffnmuls_32(data[i].op1, data[i].op2, data[i].op3, data[i].res);
    }
